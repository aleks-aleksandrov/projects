...
/*
Project 3: 
1)Implemented the mat[] in data_mem and initialized the out[]
2)Added decoding for UJ and S type
3)Made changes to the datapath to accomodate jal and jalr instructions - they are uncodnitional branchings
RUN using the project_three trace file - it includes the instructions from the project write-up
*/
...
Core *initCore(Instruction_Memory *i_mem)
{
    ...
    core->data_mem[88] = 11;
    core->data_mem[96] = 12;
    core->data_mem[104] = 13;
    core->data_mem[112] = 14;
    core->data_mem[120] = 15;
    ...
    core->reg_file[23] = 0;
    core->reg_file[20] = 0;
    core->reg_file[26] = 0; //base mat[]
    core->reg_file[25] = 128; //base out[]
...
bool tickFunc(Core *core)
{
    ...
    if(!strcmp(type, "R")){
        ...
        //printf("Decoding results = %ld %ld %ld \n", inst->rd, inst->rs_1, inst->rs_2);
    }
    else if(!strcmp(type, "I")){
        genDecode(inst,  current_instruct);
        ImmeGen(inst, current_instruct, type);
        inst->func7 = 0;
        //printf("Decoding results =  %ld %ld %ld \n", inst->rd, inst->imm, inst->rs_1);
    }
    ...
    else if(signals->Branch && alu->zero){ //Incrementing PC --> change PC based on bne output
        // printf("Branch to: %ld \n", inst->imm);
