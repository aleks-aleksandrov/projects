 ...
 * Modified: Aleksandar Aleksandrov
 * Date created: April 14, 2020
 * Date modified: January 4, 2022 
 *
 * Student names: Aleksandar Aleksandrov, Brett Chow
...
void compute_using_pthreads_v1(float *x, float *y, float a, int num_elements, int num_threads)
{
    ...
    int chunk_size = (int)floor((float)num_elements/(float)num_threads);            /* Compute the chunk size --> floor of num_el/num_th*/

    //Generate thread array of structures
    ...
    //Fork all threads and pass the necessary attributes
    for (i = 0; i < num_threads; i++)
        pthread_create(&thread_id[i], &attributes, chunk_saxpy, (void *)&thread_data[i]);
...
void compute_using_pthreads_v2(float *x, float *y, float a, int num_elements, int num_threads)
{
    ...
    for (i = 0; i < num_threads; i++)
        pthread_create(&thread_id[i], &attributes, strider_saxpy, (void *)&thread_data[i]);
...
void *chunk_saxpy(void *args){ //float *x, float *y, float a, int offset, int chunk_size, int tid
    int i;
    thread_data_t *thread_data = (thread_data_t *)args; 


    if (thread_data->tid < (thread_data->num_threads - 1)) {
        for (i = thread_data->offset; i <(thread_data->chunk_size + thread_data->offset); i++)
            thread_data->y[i] = thread_data->a*thread_data->x[i] + thread_data->y[i]; 
    }else{ //Take care of the remainder
...
void *strider_saxpy(void *args){
    ...
    /* Typecast argument to pointer to thread_data_t structure */
    thread_data_t *thread_data = (thread_data_t *)args; 
