...
#include "xdebug.h"
#include <stdio.h>
#include "xscutimer.h" //add scutimer
#include "Xtime_l.h"
 ...
 */
#define NUMBER_OF_BDS_PER_PKT		2//12
#define NUMBER_OF_PKTS_TO_TRANSFER 	1//reset per packet
...
int main(void)
{
	...
	XTime tStart, tEnd; //use xtime to time
	...
	XTime_GetTime(&tEnd); //---------Stop Timer

	printf("Accumulator took:  %f seconds.\n\r", 1.0 * (tEnd - tStart)/(COUNTS_PER_SECOND)); //print seconds
	...
	if (Error) {
		...
		Status = CheckData(MAX_PKT_LEN/4 * NUMBER_OF_BDS_TO_TRANSFER, 0xC);
...
//			return XST_FAILURE;
//		}
		whole = RxPacket[Index];
		thousandths = (RxPacket[Index] - whole) * 1000;
		xil_printf("%d.%3d\n\r", whole, thousandths);
		Value = (Value + 1) & 0xFF;
	}

//	xil_printf("Output took %llu clock cycles.\n\r", 2*(tEnd - tStart));
//	xil_printf("Output took %.2f us.\n\r", 1.0 * (tEnd - tStart) / (COUNTS_PER_SECOND));
