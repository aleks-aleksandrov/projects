...
u32 delayBufL[REC_SAMPLES]; /* Delay audio data */
u32 delayBufR[REC_SAMPLES];

//volatile 	u32 *writePtrL = delayBufL;
//volatile    u32 *writePtrR = delayBufR;
/* ------------------------------------------------------------ */
...
int AudioRunDemo(u32 i2sAddr, u32 uartAddr, u32 swAddr, u32 btnAddr)
{
	...
	Xil_Out32(i2sAddr + I2S_CTRL_REG, 0b001); // Disable mute
	while (userInput != 'q')
	{
		AudioPrintMenu();
		...
		case '4':
			...
			break;
		case '5':
			AudioDelay(i2sAddr, uartAddr);
			break;
		case '6':
			AudioDelayFeedback(i2sAddr, uartAddr);
			break;
		case 'q':
			break;
		default :
...
int AudioPlayTone(u32 i2sAddr, u32 uartAddr, u32 swAddr, u32 btnAddr)
{
	...
	while (userInput != 'q')
	{
		...
		{
			/*if (Index < 48) {
				I2SFifoWrite(i2sAddr, (1 << 31)); //Left channel
				I2SFifoWrite(i2sAddr, (1 << 31)); //Right channel
			}*/
			...
			{   // generating square wave
				if (swState & 0b10) //Left channel
					I2SFifoWrite(i2sAddr, (1 << 24));// shift 1 left 31
				else
					I2SFifoWrite(i2sAddr, 0);

				if (swState & 0b01) //Right channel
					I2SFifoWrite(i2sAddr, (1 << 24));
				else
					I2SFifoWrite(i2sAddr, 0);
			}
			else
...
int AudioRec(u32 i2sAddr, u8 useMic)
{
	...
	Xil_Out32(i2sAddr + I2S_CTRL_REG, 0b010); //Enable RX FIFO, disable TX FIFO
...
int AudioPlayRec(u32 i2sAddr)
{
	 ...
	 */
	Xil_Out32(i2sAddr + I2S_RESET_REG, 0b010); //Reset TX FIFO
	Xil_Out32(i2sAddr + I2S_CTRL_REG, 0b001); //Enable TX FIFO
	...
	Xil_Out32(i2sAddr + I2S_CTRL_REG, 0b00); //Disable RX TX FIFO
	xil_printf("Done!\n\r");
...
int AudioDelay(u32 i2sAddr, u32 uartAddr)
{
	// set microphone input
	if (AudioRegSet(&Iic, 4, 0b000010100) != XST_SUCCESS)
		return XST_FAILURE;
	// UART input
	u32 userInput = 0;
	// Flush user input UART buffer
	while (XUartPs_IsReceiveData(uartAddr))
	{
		XUartPs_ReadReg(uartAddr, XUARTPS_FIFO_OFFSET);
	}
	 // Reset RX TX FIFOs and enable them
	Xil_Out32(i2sAddr + I2S_RESET_REG, 0b110); //Reset RX TX FIFOs
	Xil_Out32(i2sAddr + I2S_CTRL_REG, 0b011);  //Enable RX TX FIFO

	while (userInput != 'q'){

			I2SFifoWrite(i2sAddr, I2SFifoRead(i2sAddr));
			I2SFifoWrite(i2sAddr, I2SFifoRead(i2sAddr));
		...
		xil_printf("Done!\n\r");
		Xil_Out32(i2sAddr + I2S_CTRL_REG, 0b00); //Disable FIFOs
		TimerDelay(500000);

		return XST_SUCCESS;
}

int AudioDelayFeedback(u32 i2sAddr, u32 uartAddr) //ADD CIRCULAR DELAY HERE
{
	// set microphone input
	if (AudioRegSet(&Iic, 4, 0b000010100) != XST_SUCCESS)
		return XST_FAILURE;
	...
	// Initialize to 0, so that previously recorded data is not played back.
	for (i = 0; i < 96000; ++i){
		delayBufL[i] = 0;
		delayBufR[i] = 0;
	}
	...
	{
		// circular buffer
		temp = (u32)((int)I2SFifoRead(i2sAddr) >> 1);
		temp2= (u32)((int)delayBufL[count] >> 1);
		delayBufL[count] = temp + temp2;
		temp = (u32)((int)I2SFifoRead(i2sAddr) >> 1);
		temp2= (u32)((int)delayBufR[count] >> 1);
		delayBufR[count] = temp + temp2;

		I2SFifoWrite(i2sAddr, delayBufL[count]);
		I2SFifoWrite(i2sAddr, delayBufR[count]);

		count++;    // count keeps track of the circular buffer index
		if ( count >= 96000 ){ // DELAY = 48000 samples
			count=0;
		}
		// end circular buffer
