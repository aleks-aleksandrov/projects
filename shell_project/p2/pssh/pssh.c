...
void handler(int sig){
    ...
    switch(sig){
        ...
        case SIGTTOU:
            ...
            while(tcgetpgrp(STDOUT_FILENO) != getpgrp()){ //pause until parent has the foreground back
                pause();
            }
            break;
        ...
        case SIGCHLD:
            ...
            while( (chld = waitpid(-1, &status, WNOHANG | WCONTINUED| WUNTRACED)) > 0 ) { //wait on children
                ...
                if (WIFCONTINUED(status)) {
                    ...
                    if(!check_job_status(pgid)){
                        set_fg_pgrp(0);
                        //printf("Job is done executing - giving terminal the fg! \n");
                    }
...
int exec_cmd(char *cmd, char**options, int pip_read, int pip_write, int num, pid_t *pid_0, int bg){
    ...
    if(num == 0){//this is the first forked child
...
void execute_tasks (Parse* P,  char* cmdline)
{
    ...
    if(command_found (P->tasks[0].cmd) || is_builtin (P->tasks[0].cmd)){ //checks if first command is supported

        if(!strcmp (P->tasks[0].cmd, "exit")){ //implement builtin exit command to exit the program
            printf("Exiting pssh...\n");
            exit(EXIT_SUCCESS);
        }
        if(!strcmp (P->tasks[0].cmd, "jobs")){ //jobs command
            for(int i = 0; i<job_num; i++){
                print_job(jobs[i]);
            }
            return;//no need to fork
        }
        if(!strcmp (P->tasks[0].cmd, "kill")){ //jobs command
            kill_builtin(P->tasks[0].argv);
            return;//no need to fork
        }

        if(!strcmp (P->tasks[0].cmd, "fg")){ //jobs command
            fg(P->tasks[0].argv);
            return;//no need to fork
        }

        if(!strcmp (P->tasks[0].cmd, "bg")){ //jobs command
            bg(P->tasks[0].argv);
            return;//no need to fork
        }
...
int main (int argc, char** argv)
{
    ...
    if (isatty(STDOUT_FILENO)){ //Store terminal
...
int check_pid(int pid){
    ...
    kill(pid, 0);
    if(errno != 0){//custom print statement for the three cases
...
void kill_builtin(char** argv){
    ...
    else if(argc == 2){// ./signal 1289 format

        pid_t pid;

        if(argv[1][0] == '%'){//Job id was supplied
            status = get_job_pgid(argv[1]);
            if(!status){
                return;
            }
            pid = status - 2*status; //make it negative for kill
        }else{
            pid = atoi(argv[1]);
        }
    ...
    else if(argc == 4 && !strcmp(argv[1], "-s")){// -->kill [-s <signal>] <pid> | %%<job>
        pid_t pid; 

        if(argv[3][0] == '%'){//Job id was supplied
...
//Sets a terminated child pid to 0 in a job structure
int remove_child(int chld_pid){
    int i,n;
    
    //printf("Removing child pid %d \n", chld_pid);
    for(i = 0; i < job_num; i++){//go through all the jobs

        for(n = 0; n < jobs[i]->npids; n++){ //go through child pids of that job
...
int check_job_status(int pgid){
    ...
    for(i = 0; i < job_num; i++){//go through all the jobs
        ...
        if(jobs[i]->pgid == pgid){//found the job
            for(n = 0; n < jobs[i]->npids; n++){ //go through child pids of that job
...
void print_job(Job *job){
    ...
    if(job->status == 1){//do not print terminated jobs
        return;
    }
    char *job_status;
    if(job->status == 0){   
